...
const {
  checkPatient, bedNameValidator, checkIfSectionIdExist, checkIfBedNotExists,
} = require('../error/bedsErrorHandler.ts');
...
export async function updateBed(req, res) {
  ...
  if (await bedNameValidator(name) && name !== undefined) {
    res.status(403).send({
      error: 'Bed name already exists',
    });
    return;
