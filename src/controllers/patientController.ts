...
const {
  checkIfSsnExists, checkValidName, patientIdValidator, patientSsnValidator,
} = require('../error/patientErrorHandler.ts');
...
export async function createPatient(req, res) {
  const {
    age, sex, first_name, last_name, ssn, additional_informations, bedId, diagnosis,
  } = req.body;
...
export async function updatePatientData(req, res) {
  const {
    first_name, last_name, sex, age, diagnosis, additional_informations, patientId,
  } = req.body;
  try {
    if (await patientIdValidator(patientId)) {
      res.status(404).send({
