...
const NecessaryInfoContext = ({ children }) => {
  ...
  const [showPopUpSucessfullyCreated, setShowPopUpSucessfullyCreated] =
    useState(false);
  const [showPopUpSuccesfullyDeleted, setShowPopUpSuccesfullyDeleted] =
    useState(false);
  const [showPopUpOccupied, setShowPopUpOccupied] = useState(false);
  const [showPopUpSuccessfullyAvaible, setShowPopUpSuccessfullyAvaible] =
    useState(false);
  const [showPopUpViewInfoPatient, setShowPopUpViewInfoPatient] =
    useState(false);
  const [
